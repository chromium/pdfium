...
public:
    ...
    void FFI_EmailTo(FPDF_FILEHANDLER* fileHandler, FPDF_WIDESTRING pTo,
                     FPDF_WIDESTRING pSubject, FPDF_WIDESTRING pCC,
                     FPDF_WIDESTRING pBcc, FPDF_WIDESTRING pMsg)
    {
                                      ...
                                      const FX_WCHAR* wsHeader)
    {
        if (m_pInfo && m_pInfo->FFI_PostRequestURL)
        {
            ...
            m_pInfo->FFI_PostRequestURL(m_pInfo, URL, data, contentType, encode, header, &respone);
