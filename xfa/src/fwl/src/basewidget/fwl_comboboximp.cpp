...
CFWL_ComboEdit::CFWL_ComboEdit(const CFWL_WidgetImpProperties& properties,
                               IFWL_Widget* pOuter)
...
CFWL_ComboList::CFWL_ComboList(const CFWL_WidgetImpProperties& properties,
                               IFWL_Widget* pOuter)
...
void CFWL_ComboListDelegate::OnDropListFocusChanged(CFWL_Message* pMsg,
                                                    FX_BOOL bSet) {
  if (!bSet) {
...
FWL_ERR CFWL_ComboBoxImp::Update() {
  ...
  m_fComboFormHandler =
      *(FX_FLOAT*)m_pProperties->m_pThemeProvider->GetCapacity(
...
FWL_ERR CFWL_ComboBoxImp::DrawWidget(CFX_Graphics* pGraphics,
  ...
  if (!bIsDropDown) {
    ...
    if (m_iCurSel >= 0) {
      IFWL_ListBoxDP* pData =
          (IFWL_ListBoxDP*)((CFWL_ComboList*)((IFWL_TargetData*)m_pListBox)
                                ->GetData())
    ...
    if (m_iCurSel >= 0) {
      ...
      ((CFWL_ComboList*)((IFWL_TargetData*)m_pListBox)->GetData())
...
FWL_ERR CFWL_ComboBoxImp::SetCurSel(int32_t iSel) {
  int32_t iCount = ((CFWL_ComboList*)((IFWL_TargetData*)m_pListBox)->GetData())
                       ->CountItems();
...
FX_FLOAT CFWL_ComboBoxImp::GetListHeight() {
  return ((IFWL_ComboBoxDP*)m_pProperties->m_pDataProvider)
...
void CFWL_ComboBoxImp::ShowDropList(FX_BOOL bActivate) {
  ...
  if (bActivate) {
    ...
    DispatchEvent(&ev);
    m_fItemHeight = ((CFWL_ComboList*)((IFWL_TargetData*)m_pListBox)->GetData())
                        ->m_fItemHeight;
    ((CFWL_ComboList*)((IFWL_TargetData*)m_pListBox)->GetData())
        ->SetFocus(TRUE);
    m_pForm->DoModal();
...
void CFWL_ComboBoxImp::MatchEditText() {
  ...
  int32_t iMatch = ((CFWL_ComboList*)((IFWL_TargetData*)m_pListBox)->GetData())
                       ->MatchItem(wsText);
  if (iMatch != m_iCurSel) {
    ((CFWL_ComboList*)((IFWL_TargetData*)m_pListBox)->GetData())
  ...
  if (bDropDown) {
    ...
    if (hItem) {
      ...
      if (m_pEdit) {
        m_pEdit->SetText(wsText);
        m_pEdit->Update();
        ((CFWL_ComboEdit*)((IFWL_TargetData*)m_pEdit)->GetData())
            ->SetSelected();
      }
...
void CFWL_ComboBoxImpDelegate::OnFocusChanged(CFWL_Message* pMsg,
  ...
  if (bSet) {
    ...
    if (bDropDown && pSrcTarget != (IFWL_Widget*)m_pOwner->m_pListBox) {
      ...
      ((CFWL_ComboEdit*)((IFWL_TargetData*)m_pOwner->m_pEdit)->GetData())
          ->SetSelected();
    } else {
...
void CFWL_ComboBoxImpDelegate::DoSubCtrlKey(CFWL_MsgKey* pMsg) {
  ...
  if (bUp || bDown) {
    int32_t iCount =
        ((CFWL_ComboList*)((IFWL_TargetData*)m_pOwner->m_pListBox)->GetData())
            ->CountItems();
    if (iCount < 1) {
      return;
    }
...
void CFWL_ComboBoxImpDelegate::DisForm_OnKey(CFWL_MsgKey* pMsg) {
  ...
  if (bUp || bDown) {
    CFWL_ComboList* pComboList =
        ((CFWL_ComboList*)((IFWL_TargetData*)(m_pOwner->m_pListBox))
             ->GetData());
...
int32_t CFWL_ComboProxyImpDelegate::OnProcessMessage(CFWL_Message* pMessage) {
  ...
  if (dwMsgCode == FWL_MSGHASH_KillFocus || dwMsgCode == FWL_MSGHASH_SetFocus) {
    OnFocusChanged((CFWL_MsgKillFocus*)pMessage,
